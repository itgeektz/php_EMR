...
$primary_docs = array(
		...
		'cus_doc' => array(xl('Custom Reports') , 2, 'super/custom_reports.php'),
);
...
$query='SELECT ag.name FROM  gacl_groups_aro_map gam  LEFT JOIN `gacl_aro` ga  ON gam.aro_id=ga.id  LEFT JOIN gacl_aro_groups ag on ag.id=gam.group_id  where  ga.value="'.$_SESSION['authUser'].'"';

$res = sqlStatement($query);
while ($row = sqlFetchArray($res)) {
	$_SESSION['usergroup']=$row['name'];
}
...
$disallowed['adm'] = !(acl_check('admin', 'calendar') || acl_check('admin', 'database') || acl_check('admin', 'forms') ||acl_check('admin', 'practice') || acl_check('admin', 'users') ||
...
$disallowed['iss'] = !((acl_check('encounters','notes','','write') ||acl_check('encounters','notes_a','','write') ) &&acl_check('patients','med','','write') );
...
$GLOBALS["usergroup_list"]=array("Physicians");

//for Physicians
//$GLOBALS["menu_lsit"]=array("pwd","adm","cal","new"," dem","nen","ens","prq","cus_doc");

//for Front Office
$GLOBALS["menu_lsit"]=array("cal","pwd","dem","adm","cal","new","dem","nen","ens","prq","cus_doc");
...
function genTreeLink($frame, $name, $title, $mono=false) {
	...
	// echo $name;
	//     if(!in_array($name,$GLOBALS["menu_lsit"]))
	//       return ;
...
<script language='JavaScript'>
 ...
 function loadFrame3(fname, frame, url) 
 {
  ...
  if (!f.cb_bot.checked)
	 frame = 'RTop'; else if (!f.cb_top.checked) frame = 'RBot';

  top.frames[frame].location = '<?php echo "$web_root/interface/" ?>' + url;
  if (frame == 'RTop')
	  TopName = fname; else botName = fname;
  return false;
 }
...
<?php
	...
	if(in_array($_SESSION['usergroup'], $limit_access_group))
		...
		?>	
			$frame1url= '../main/finder/dynamic_finder.php';
		<?php 		
		}else
			{
			?>	
...
function clearactive() {
	  ...
	  success:function( msg ) {
		...
		<?php
				...
				if(in_array($_SESSION['usergroup'], $limit_access_group))
					...
					?>	
					top.frames['RTop'].location= '../main/finder/dynamic_finder.php';
					<?php 		
					}else
						{
						?>	
	...
	$(parent.Title.document.getElementById('pat_cat')).hide();
...
function setPatientCategory(pat_cat){

	setTitleContent('patient_cate',pat_cat);
 ...
 function setPatient(pname, pid, pubpid, frname, str_dob) {
  ...
  reloadIssues(pid);
  $(parent.Title.document.getElementById('pat_cat')).show();
...
function isEncounterLocked( encounterId ) {
 ...
 function loadCurrentEncounterFromTitle() {
      ...
      top.frames[ parent.left_nav.getEncounterTargetFrame('enc') ].location='../patient_file/summary/demographics.php?opener_save=true';
 }
...
?>
  ...
  <li class="open"><a class="collapsed" id="patimg" ><span><?php xl('Medical Administration','e') ?></span></a>
    ...
    <?php 
      if(!in_array($_SESSION['usergroup'],$GLOBALS["usergroup_list"]))
      {
    ?>
  ...
  <?php 
         ...
         if(in_array($_SESSION['usergroup'],$GLOBALS["usergroup_list"]))
          {
              $GLOBALS['enable_fees_in_left_menu']=0;
          }
  ...
  <li><a class="collapsed" id="misimg" ><span><?php xl('Miscellaneous','e') ?></span></a>
    <ul>
      <?php if(acl_check('patients','docs')) genTreeLink('RBot','aun',xl('Authorizations')); ?>
...
<?php } else { // not athletic team ?>
  <?php if (!$GLOBALS['disable_calendar'] && !$GLOBALS['ippf_specific'] && acl_check('calendar','cal_calendar')) genTreeLink('RTop','cal',xl('Calendar')); ?>
  <?php if (!$GLOBALS['disable_pat_trkr'] && !$GLOBALS['disable_calendar']) genTreeLink('RTop','pfb',xl('Flow Board')); ?>
  <?php if(acl_check('calendar','cal_messages')) genTreeLink('RBot','msg',xl('Messages')); ?>
  ...
  <?php 
    ...
    //echo "asds";exit;
    $PatientClient=acl_check('patientmenu', 'pat_patient');
    $NewSearch=acl_check('patientmenu', 'pat_addsrc');
    $Summary=acl_check('patientmenu', 'pat_sumry');
    $CreateVisit=acl_check('patientmenu', 'pat_vsit_creat');
    $VisitHistory=acl_check('patientmenu', 'pat_vsit_histry');
    $PatientRecordRequest=acl_check('patientmenu', 'pat_recod_patrecreq');
    $AddNew=acl_check('patientmenu', 'pat_custmrep_addnew');
    $VisitForms=acl_check('patientmenu', 'pat_vsitfrm');
    $Import=acl_check('patientmenu', 'pat_import');


    if($PatientClient || $NewSearch || $Summary || $CreateVisit || $VisitHistory || $PatientRecordRequest || $AddNew || $VisitForms || $Import)
    {
  ?>

  <li class="open"><a class="expanded" id="patimg" ><span><?php xl('Patient/Client','e') ?></span></a>
    <ul>
      <?php if($PatientClient) genMiscLink('RTop','fin','0',xl('Patients'),'main/finder/dynamic_finder.php'); ?>
      <?php if($NewSearch)  genTreeLink('RTop','new',($GLOBALS['full_new_patient_form'] ? xl('New/Search') : xl('New'))); ?>
      <?php if($Summary)  genTreeLink('RTop','dem',xl('Summary')); ?>
      <?php 
        if($CreateVisit || $VisitHistory)
        {
      ?>
      <li class="open"><a class="expanded_lv2"><span><?php xl('Visits','e') ?></span></a>
        <ul>
          ...
          <?php if($CreateVisit) genTreeLink('RBot','nen',xl('Create Visit')); ?>
          <?php if($VisitHistory) genTreeLink('RBot','ens',xl('Visit History')); ?>
        </ul>
      </li>
      <?php
        }
      ...
      <li><a class="collapsed_lv2"><span><?php xl('Records','e') ?></span></a>
        <ul>
          <?php if($PatientRecordRequest) genTreeLink('RTop','prq',xl('Patient Record Request')); ?>
        </ul>
      </li>
      <?php 
        } 
        ...
        {
      ?>
       <li><a class="collapsed_lv2"><span><?php xl('Custom Reports','e') ?></span></a>
        <ul>
          <?php if($AddNew) genTreeLink('RTop','cus_doc',xl('Add New')); 
          ?>
        </ul>
      </li>
      <?php 
      ...
      ?>
<?php if ($GLOBALS['gbl_nav_visit_forms'] && $VisitForms && $Import) { ?>
...
<?php } // end if gbl_nav_visit_forms ?>
  ...
  <?php 
    ...
    if((!isset($GLOBALS['enable_fees_in_left_menu']) || $GLOBALS['enable_fees_in_left_menu'] == 1) && (acl_check('acct', 'bill') || acl_check('acct', 'eob') || acl_check('acct', 'disc') || acl_check('acct', 'eob') || acl_check('acct', 'rep')|| acl_check('acct', 'rep_a'))){ ?>
  <li><a class="collapsed" id="feeimg" ><span><?php xl('Fees','e') ?></span></a>
    <ul>
      <?php if(acl_check('acct', 'bill')) genMiscLink('RBot','cod','2',xl('Fee Sheet'),'patient_file/encounter/load_form.php?formname=fee_sheet'); ?>
      <?php if ($GLOBALS['use_charges_panel']  && acl_check('acct', 'bill')) genTreeLink('RBot','cod',xl('Charges')); ?>
      <?php  if(acl_check('acct', 'bill')) genMiscLink('RBot','pay','1',xl('Payment'),'patient_file/front_payment.php'); ?>
      <?php  if(acl_check('acct', 'bill'))genMiscLink('RBot','bil','1',xl('Checkout'),'patient_file/pos_checkout.php?framed=1'); ?>
      <?php if (! $GLOBALS['simplified_demographics'] &&  acl_check('acct', 'eob')) genTreeLink('RTop','bil',xl('Billing')); ?>
	  <?php  if(acl_check('acct', 'bill')) genTreeLink('RTop','npa',xl('Batch Payments'),false,2);?>
	  <?php if(acl_check('acct', 'bill')) genMiscLink('RTop','eob','0',xl('Posting'), 'billing/sl_eob_search.php'); ?>
...
<?php } ?>
   ...
   <?php 
      $Providers=acl_check('procedures', 'pro_prvid');
      $Configuration=acl_check('procedures', 'pro_conf');
      $LoadCompendium=acl_check('procedures', 'pro_lodcomp');
      $PendingReview=acl_check('procedures', 'pro_pendreviw');
      $PatientResults=acl_check('procedures', 'pro_patntres');
      $LabOverview=acl_check('procedures', 'pro_labovrviw');
      $BatchResults=acl_check('procedures', 'pro_batres');
      $ElectronicReports=acl_check('procedures', 'pro_electrep');
      $LabDocuments=acl_check('procedures', 'pro_labdoc');

      

     if($Providers || $Configuration || $LoadCompendium || $PendingReview || $PatientResults  || $LabOverview || $BatchResults || $ElectronicReports || $LabDocuments){
   ?> 
   <li><a class="collapsed" id="proimg" ><span><?php xl('Procedures','e') ?></span></a>
    <ul>
      <?php if($Providers) genTreeLink('RTop','orl',xl('Providers')); ?>
      <?php if($Configuration) genTreeLink('RTop','ort',xl('Configuration')); ?>
      <?php if($LoadCompendium) genTreeLink('RTop','orc',xl('Load Compendium')); ?>
      <?php if($PendingReview) genTreeLink('RTop','orp',xl('Pending Review')); ?>
      <?php if($PatientResults) genTreeLink('RTop','orr',xl('Patient Results')); ?>
      <?php if($LabOverview) genTreeLink('RTop','lda',xl('Lab Overview')); ?>
      <?php if($BatchResults) genTreeLink('RTop','orb',xl('Batch Results')); ?>
      <?php if($ElectronicReports) genTreeLink('RTop','ore',xl('Electronic Reports')); ?>
      <?php if($LabDocuments) genTreeLink('RTop','dld',xl('Lab Documents'));?>
    </ul>
  </li>
  <?php } ?>
  <?php
  ...
  <li><a class="collapsed" id="admimg" ><span><?php xl('Administration','e') ?></span></a>
    <ul>
      ...
      <?php if (acl_check('admin', 'super'    )) genMiscLink('RTop','adm','0',xl('Import Rate Master'),'import_rate_master.php'); ?>
      <?php if (acl_check('admin', 'super'    )) genMiscLink('RTop','adm','0',xl('Import Rate & Patient Master'),'import_rate_patient_master.php'); ?>
      <?php if (acl_check('admin', 'super'    )) genMiscLink('RTop','adm','0',xl('Import Pharmacy Rate Master'),'import_pharmacy_rate_master.php'); ?>
      <?php if (acl_check('admin', 'super'    )) genMiscLink('RTop','adm','0',xl('Import Pharmacy Rate & Patient Master'),'import_pharmacy_rate_patient_master.php'); ?>
      <?php if ( ($GLOBALS['include_de_identification']) && (acl_check('admin', 'super'    )) ) genMiscLink('RTop','adm','0',xl('De Identification'),'de_identification_forms/de_identification_screen1.php'); ?>
          <?php if ( ($GLOBALS['include_de_identification']) && (acl_check('admin', 'super'    )) ) genMiscLink('RTop','adm','0',xl('Re Identification'),'de_identification_forms/re_identification_input_screen.php'); ?>
  ...
  <?php 
    $Clients=acl_check('reports', 'rep_client');
    $Clinic=acl_check('reports', 'rep_clinic');
    $Visits=acl_check('reports', 'rep_visit');
    $Financial=acl_check('reports', 'rep_finance');
    $Procedures=acl_check('reports', 'rep_procedur');
    $Insurance=acl_check('reports', 'rep_insurnce');
    $BlankForms=acl_check('reports', 'rep_blnkfrm');
    $Services=acl_check('reports', 'rep_servc');
    $Statistics=acl_check('reports', 'rep_statistic');
  
    if($Clients || $Clinic  || $Visits  || $Financial  || $Procedures  || $Insurance  || $BlankForms  || $Services  || $Statistics)
    {
  ?>
  ...
  <li><a class="collapsed" id="repimg" ><span><?php xl('Reports','e') ?></span></a>
    <ul>
      ...
      ?>
  	  ...
  	  <?php genMiscLink('RTop','rep','0',xl('Final Diagnosis Report'),'reports/final_diagnosis_report.php'); ?>
          </ul>
        </li>
      <?php 
      }
      if($Clinic && ($GLOBALS['enable_cdr'] || $GLOBALS['enable_cqm']  || $GLOBALS['enable_amc']))
      {
      ?> 
...
?>
  ...
  <li><a class="collapsed" id="misimg" ><span><?php xl('Miscellaneous','e') ?></span></a>
    <ul>
      <?php if(acl_check('Miscellaneous','mis_ped')) genTreeLink('RTop','ped',xl('Patient Education')); ?>
      <?php if(acl_check('Miscellaneous','mis_autoriz')) genTreeLink('RBot','aun',xl('Authorizations')); ?>
      <?php if(acl_check('Miscellaneous','mis_adb'))  genTreeLink('RTop','adb',xl('Addr Book')); ?>
      <?php  if(acl_check('Miscellaneous','mis_ordcat'))  genTreeLink('RTop','ort',xl('Order Catalog')); ?>
      <?php if(acl_check('Miscellaneous','mis_chattrak')) if (!$GLOBALS['disable_chart_tracker']) genTreeLink('RTop','cht',xl('Chart Tracker')); ?>
      <?php if(acl_check('Miscellaneous','mis_ofcnot')) genTreeLink('RTop','ono',xl('Ofc Notes')); ?>
      <?php if(acl_check('Miscellaneous','mis_bachcom')) genMiscLink('RTop','adm','0',xl('BatchCom'),'batchcom/batchcom.php'); ?>
      ...
      <?php if(acl_check('Miscellaneous','mis_pass')) genTreeLink('RTop','pwd',xl('Password')); ?>
      <?php if(acl_check('Miscellaneous','mis_prfrnce')) genMiscLink('RTop','prf','0',xl('Preferences'),'super/edit_globals.php?mode=user'); ?>
      <?php if(acl_check('patients','docs')){  $disallowed["adm"]=0; genMiscLink('RTop','adm','0',xl('New Documents'),'../controller.php?document&list&patient_id'); $disallowed["adm"]=1; } ?>
       <?php if (acl_check('patients','docs') && acl_check('Miscellaneous','mis_custdoc')){ $disallowed["adm"]=0; genMiscLink('RTop','adm','0',xl('Custom Documents'),'super/custom_document_templates.php'); $disallowed["adm"]=1; }?>
      <?php if (acl_check('patients','docs') || in_array($_SESSION['usergroup'],$GLOBALS["usergroup_list"])){ $disallowed["adm"]=0; genMiscLink('RTop','adm','0',xl('Document Templates'),'super/manage_document_templates.php'); $disallowed["adm"]=1;}?>
...
<script>
    var group="<?php echo $_SESSION['usergroup'] ;?>";
  
  $(document).ready(function (){
        if(group=='Physicians')
          $("#fin0").click();
        else if(group=='Front Office')
          $("#fin0").click();
        else if(group=='DataEntry User')
          $("#fin0").click();
