...
if (isset($_GET['iSortCol_0'])) {
    ...
    $iSortCol = intval($_GET["iSortCol_$i"]);
      ...
      } elseif ($aColumns[$iSortCol] == 'bill_time') {
        $orderby .= "UNIX_TIMESTAMP(bill_time) $sSortDir";
      }
      else {
  ...
  if (isset($_GET["bSearchable_$i"]) && $_GET["bSearchable_$i"] == "true" && $_GET["sSearch_$i"] != '') {
    ...
    if ($colname == 'name') {
      $where .= " ( " .
        ...
        "fname LIKE '$sSearch%' OR " .
        "mname LIKE '$sSearch%' )";
    } elseif ($colname == 'bill_time') {
        $tmp = str_replace('/', '', $sSearch);;
        $where .= " DATE_FORMAT($colname,'%d%m%Y%r') LIKE '%$tmp%'";
    } elseif ($colname == 'DOB') {
        $tmp = str_replace('/', '', $sSearch);;
        $where .= " DATE_FORMAT($colname,'%d%m%Y%r') LIKE '%$tmp%'";
    } elseif ($colname == 'doc_name') {
        $tmp = str_replace('/', '', $sSearch);;
        $where .= " doc_name LIKE '%$tmp%'";
    }
    else {
      $where .= " `" . escape_sql_column_name($colname,array('patient_data')) . "` LIKE '$sSearch%'";
    }
...
foreach ($aColumns as $colname) {
  ...
  if ($colname == 'name') {
    ...
    $sellist .= "DATE_FORMAT(bill_time,'%d/%m/%Y %r') as bill_time";
  }
  else {
