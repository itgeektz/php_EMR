            ...
            // ===== Future Recurring events of a repeating series =====
                ...
                foreach ($_POST['form_provider'] as $provider) {
                    sqlStatement("UPDATE openemr_postcalendar_events SET " .
                        ...
                        "pc_pname = '" . add_escape_custom($_POST['pc_pname']) . "', " .
						"pc_pnumber = '" . add_escape_custom($_POST['pc_pnumber']) . "', " .
						"pc_page = '" . add_escape_custom($_POST['pc_page']) . "', " .
						"pc_psex = '" . add_escape_custom($_POST['pc_psex']) . "', " .
						"pc_plocation = '" . add_escape_custom($_POST['pc_plocation']) . "', " .
						"pc_pemail = '" . add_escape_custom($_POST['pc_pemail']) . "', " .
...
<?php require($GLOBALS['srcdir'] . "/restoreSession.php"); ?>
 ...
 function sel_patient() {
  dlgopen('find_patient_popup_search.php', '_blank', 1000, 600);
 }
...
<?php echo $catoptions ?>
 ...
 <tr id="patient_details" class="sel_patient">
  <td nowrap>
   <b><?php echo xlt('Patient'); ?>:</b>
  </td>
  <td nowrap>
   <input type='text' size='10' id="form_patient" name='form_patient' style='width:100%;cursor:pointer;cursor:hand' value='<?php echo attr($patientname); ?>' onclick='sel_patient()' title='<?php echo xla('Click to select patient'); ?>' readonly />
  </td>
 ...
 <input type='hidden' id='form_pid' name='form_pid' value='<?php echo attr($patientid) ?>' />
 <tr>
  <td nowrap>
   <b><?php echo xlt('Non-register Patient'); ?>:</b>
  </td>
  <td nowrap>
  	<input type="checkbox" name="add_patient" id="add_patient" value="1">
  </td>
 </tr> 
 <tr style="display:none" class="add_patient_details">
	<td nowrap>
		 <b><?php echo xlt('Patient Name'); ?>:</b>
	</td>
	  <td nowrap>
		<input type="text" name="pc_pname" id="form_pname" value="<?php echo attr($row['pc_pname']) ?>">
	  </td>
	 </tr> 
  <tr style="display:none" class="add_patient_details">
	  <td nowrap>
	   <b><?php echo xlt('Patient Number'); ?>:</b>
	  </td>
	  <td nowrap>
		<input type="text" name="pc_pnumber" id="form_pnumber" value="<?php echo attr($row['pc_pnumber']) ?>">
	  </td>
  </tr> 
  <tr style="display:none" class="add_patient_details">
	  <td nowrap>
	   <b><?php echo xlt('Patient Age'); ?>:</b>
	  </td>
	  <td nowrap>
		<input type="text" name="pc_page" id="form_page" value="<?php echo attr($row['pc_page']) ?>">
	  </td>
  </tr> 
  <tr style="display:none" class="add_patient_details">
	  <td nowrap>
	   <b><?php echo xlt('Patient Sex'); ?>:</b>
	  </td>
	  <td nowrap>
		<select name="pc_psex" id="form_psex">
			<option value="Male" <?php echo (attr($row['pc_psex']) == 'Male') ? 'selected="selected"' :'' ?>><?php echo xlt('Male'); ?></option>
			<option value="Female" <?php echo (attr($row['pc_psex']) == 'Female') ? 'selected="selected"' :'' ?>><?php echo xlt('Female'); ?></option>
			<option value="Other" <?php echo (attr($row['pc_psex']) == 'Other') ? 'selected="selected"' :'' ?>><?php echo xlt('Other'); ?></option>
		</select>
	  </td>
  </tr> 
  <tr style="display:none" class="add_patient_details">
	  <td nowrap>
	   <b><?php echo xlt('Patient location'); ?>:</b>
	  </td>
	  <td nowrap>
		<input type="text" name="pc_plocation" id="form_plocation" value="<?php echo attr($row['pc_plocation']) ?>">
	  </td>
  </tr> 
  <tr style="display:none" class="add_patient_details">
	  <td nowrap>
	   <b><?php echo xlt('Patient Email'); ?>:</b>
	  </td>
	  <td nowrap>
		<input type="url" name="pc_pemail" id="form_pemail" value="<?php echo attr($row['pc_pemail']) ?>">
	  </td>
  </tr> 
...
$(document).ready(function(){
    ...
    dateChanged();
	//Add Patient
	<?php if (!empty (attr($row['pc_pname']))) { ?>
		$('.add_patient_details').show();
		$('.sel_patient').hide();
	<?php } ?>
	$('#add_patient').change(function (){
		$('.add_patient_details').toggle();
		$('.sel_patient').toggle();
		if ($(this).val()){
			$('#form_pid').val('DP');
		}
...
function validate(valu) {
    ...
    ?>
	 ...
	 if(f.form_pid.value == 'DP'){
		 if(f.form_pname.value == ''){
		  alert('<?php echo addslashes(xl('Patient Name Required'));?>');
		  return false;
		 }
		 if(f.form_pnumber.value == ''){
		  alert('<?php echo addslashes(xl('Patient Number Required'));?>');
		  return false;
		 }
		 if(f.form_page.value == ''){
		  alert('<?php echo addslashes(xl('Patient Age Required'));?>');
		  return false;
		 }
		 if(f.form_plocation.value == ''){
		  alert('<?php echo addslashes(xl('Patient Location Required'));?>');
		  return false;
		 }
