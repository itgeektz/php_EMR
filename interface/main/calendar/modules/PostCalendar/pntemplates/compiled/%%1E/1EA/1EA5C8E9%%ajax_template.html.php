<?php /* Smarty version 2.6.29, created on 2020-11-24 10:17:43
         compiled from default/views/day/ajax_template.html */ ?>
<?php require_once(SMARTY_CORE_DIR . 'core.load_plugins.php');
smarty_core_load_plugins(array('plugins' => array(array('function', 'config_load', 'default/views/day/ajax_template.html', 10, false),array('function', 'pc_sort_events', 'default/views/day/ajax_template.html', 405, false),array('modifier', 'date_format', 'default/views/day/ajax_template.html', 400, false),array('modifier', 'string_format', 'default/views/day/ajax_template.html', 401, false),)), $this); ?>
...
<!-- change by me for search box of provider / doctor list
start 
-->
<link rel="stylesheet" href="<?php  echo $GLOBALS['webroot'];  ?>/library/js/searchbox/chosen.css">
...
if (count($facilities) > 1) {
 ...
 //changed by me start related to search doctor /provider list

 $usergroup=getUserbyGroup('13');

echo "   <select multiple size='5' name='pc_usernameserch[]' id='pc_usernameserch' class='view2 chosen-select'>\n";
 echo "    <option value='__PC_ALL__'>"  .xl ("All Users"). "</option>\n";
 foreach ($provinfo as $doc) {

    $username = $doc['username'];
    if(in_array($username,$usergroup))
    {
 ...
 echo "   <select multiple size='50' name='pc_username[]' id='pc_username' class='view2'>\n";
 echo "    <option value='__PC_ALL__'>"  .xl ("All Users"). "</option>\n";
 foreach ($provinfo as $doc) {
	    ...
	    $fname = substr($event['patient_name'], $commapos + 2);
            ...
            if ($catid == 2 || $catid == 3 || $catid == 4 || $catid == 8 || $catid == 11) {
                ...
                if ($patientid) {
                   ...
                   $link_title .= htmlspecialchars($comment, ENT_QUOTES);
...
<script src="https://ajax.googleapis.com/ajax/libs/jquery/1.6.4/jquery.min.js" type="text/javascript"></script>
  <script src="<?php  echo $GLOBALS['webroot'];  ?>/library/js/searchbox/chosen.jquery.js" type="text/javascript"></script>
  <script type="text/javascript">
    ...
    $(document).ready(function(){
        $("#pc_usernameserch").change(function(){
            var sval=$("#pc_usernameserch").val().toString().split(",");
            var len=sval.length-1;
          //  alert(sval[len]);
            $.each(sval, function(i,e){
            $("#pc_username option").prop("selected", false);
});
            $.each(sval, function(i,e){
                $("#pc_username option[value='" + e + "']").prop("selected", true);
});
            $usname=$("#pc_username");
           ChangeProviders($usname);
            
        });
          $("#pc_usernameserch_chosen").css("width",'100%');
    });
  </script>
<!-- change end -->
</html>