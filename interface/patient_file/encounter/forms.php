...
?>
	<br>
	<a title="<?php echo attr($note);?>" href="<?php echo $doc_url;?>" style="font-size:small;" onsubmit="return top.restoreSession()"><?php echo oeFormatShortDate($doc_iter[docdate]) . ": " . text(basename($doc_iter[url]));?></a> <span><?php echo '(by. '.$doc_iter['autho_fname'].' '.$doc_iter['autho_lname'].')'?></span>
<?php } ?>
</div>
<?php } ?>
<br/>
...
<?php
  if ($result = getFormByEncounter($pid, $encounter, "id, date, form_id, form_name, formdir, user, deleted")) {
    ...
    foreach ($result as $iter) {
        ...
        $frm_date = strtotime($iter['date']);
        $frm_date = date("Y-m-d", $frm_date);
       ...
       if (acl_check('admin', 'super') || $frm_date == date("Y-m-d") || $frm_date == date("Y-m-d", strtotime("-1 days"))){
