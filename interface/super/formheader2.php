...
if(isset($_POST['pid']))
{
	...
	$result = getPatientData($pid, "fname,lname,file_number,pubpid,sex, DATE_FORMAT(DOB,'%Y-%m-%d') as DOB_YMD");
	//echo '<pre>';print_r($result);die; 
	$uname=sqlQuery("select fname,lname from users where id=".$_SESSION['authUserID']);
	$html.='<table width=100% id="template_table">';
	...
	$html.='<tr><td style="padding:7px;font-weight: bold;font-size: 12px;">Registration No : </td><td style="padding:7px;font-weight: bold;font-size: 12px;">'.addslashes($result['pubpid']).' </td><td style="padding:7px;font-weight: bold;font-size: 12px;width:10%">&nbsp;</td><td style="padding:7px;font-weight: bold;font-size: 12px;">Sex : </td><td style="padding:7px;font-weight: bold;font-size: 12px;">'.addslashes($result['sex']).' </td></tr>';
	...
	$html.='<tr><td style="padding:7px;font-weight: bold;font-size: 12px;">Doctor : </td><td style="padding:7px;font-weight: bold;font-size: 12px;" class="doc_name"></td></tr>';
	$html.='</table></div>';
	echo $html;
