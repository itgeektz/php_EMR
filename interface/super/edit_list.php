...
$list_id = empty($_REQUEST['list_id']) ? 'abook_type' : $_REQUEST['list_id'];
...
function writeOptionLine($option_id, $title, $seq, $default, $value, $mapping='', $notes='', $codes='',$tog1='', $tog2='', $active='',$subtype='') {
  ...
  else if ($list_id == 'drug_interval') {
    echo "  <td align='center' class='valuecell'>";
    echo "<input type='text' name='opt[$opt_line_no][value]' value='" .
        htmlspecialchars($value, ENT_QUOTES) . "' size='4' maxlength='10' class='option_value' />";
    echo "</td>\n";
  }
...
<form action='edit_list.php' method='post' name='theform' id='theform'>
...
<?php } else if($list_id == 'drug_interval'){
      ?>
        <td title='<?php xl('Option Value','e'); ?>'><b><?php xl('Value','e'); ?></b></td>
        <td><b><?php xl('Notes','e');?></b></td>
      <?php
      }  
		else if($list_id == 'language') { ?>
<td><b> <?php 	xl('ISO 639-2 Code','e');?> </b></td>
		  <?php } else if ($list_id == 'personal_relationship' || $list_id == 'religious_affiliation' || $list_id == 'ethnicity' || $list_id == 'race' || $list_id == 'drug_route'){?>
         <td><b>	 <?php
                        xl('HL7-V3 Concept Code','e');?></b></td>
                  <?php } else if ($list_id == 'Immunization_Completion_Status'){?>
         <td><b>	 <?php
                        xl('Treatment Completion Status','e');?></b></td>
		  <?php
