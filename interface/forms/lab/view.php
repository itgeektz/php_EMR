...
$obj = formFetch("form_lab_rmc", $_GET["id"]);
$codes = json_decode($obj['lab_codes']);
$codes_check = array();
$source_check = array();
  ...
  foreach ($cat_values as $key => $value)
	{
		...
		if( explode('_', $key)[0] == 'source')
		{
			
			$source_check[$key] = $value;
		}	
...
<html>
	<head>
		<?php html_header_show();?>
		 ...
		 <script src="<?php echo $GLOBALS['webroot']; ?>/library/js/jquery-3.2.1.min.js.js" type="text/javascript" ></script>
 		 <script src="<?php echo $GLOBALS['webroot']; ?>/library/js/bootstrap-3.3.7.min.js" type="text/javascript" ></script>
 		 <script src="<?php echo $GLOBALS['webroot']; ?>/library/js/mbd/mdb.min.js" type="text/javascript" ></script>
 			
		<style>
		  .demo_table tr > td, .section-header,.section-header-dynamic{height: 30px !important;}
	...
	<body class="body_top">
	 <div class="container-fluid" style="border: 1px solid silver; box-sizing: border-box;">
    	<form role="form" method = "POST" action="<?php echo $rootdir;?>/forms/lab/save.php?mode=update&id=<?php echo $_GET["id"]; ?>" name="my_form" onsubmit="return top.restoreSession()" >
...
<script type="text/javascript" >
	...
	/* 	$('#test').on('click',function(){
       ...
       var code_array = <?php echo json_encode($codes_check); ?>;

       $('input:checkbox').each(function(){

		  var state = $.inArray(this.value,code_array) != -1;

		  $(this).prop('checked',state);
		});


       <?php foreach ($source_check as $key => $value){
       			if($key == 'source_1')
       			{	
       	?>
					$('.source_1').val(<?php echo json_encode($value); ?>);
   				<?php }elseif($key == 'source_2'){?>

   				$('.source_2').val(<?php echo json_encode($value); ?>);
		<?php }}?>
