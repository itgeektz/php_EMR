...
?>
	...
	if(opener)
  {
   ...
   opener.window.top.left_nav.setPatientEncounter(EncounterIdArray,EncounterDateArray,CalendarCategoryArray);
<?php } ?>
 opener.window.top.restoreSession();
<?php if ($GLOBALS['concurrent_layout']) { ?>
<?php if ($mode == 'new') { ?>
 opener.window.top.left_nav.setEncounter(<?php echo "'" . attr(oeFormatShortDate($date)) . "', " . attr($encounter) . ", window.name"; ?>);
 opener.window.top.left_nav.setRadio(window.name, 'enc');
<?php } // end if new encounter ?>
 //opener.window.top.left_nav.loadFrame('enc2', 'RTop', '<?php echo $nexturl; ?>');

 <?php $_SESSION['opener_save'] = "true"; ?>

 window.close();

 opener.window.top.left_nav.setRadio(window.name, 'dem');
 opener.window.top.left_nav.loadFrame2('dem1', window.name, 'patient_file/summary/demographics.php?frm_enc=true&visit=1');

  }
  else
  {
   window.top.left_nav.setPatientEncounter(EncounterIdArray,EncounterDateArray,CalendarCategoryArray);
<?php } ?>
 window.top.restoreSession();
<?php if ($GLOBALS['concurrent_layout']) { ?>
<?php if ($mode == 'new') { ?>
 window.top.left_nav.setEncounter(<?php echo "'" . attr(oeFormatShortDate($date)) . "', " . attr($encounter) . ", window.name"; ?>);
 window.top.left_nav.setRadio(window.name, 'enc');
<?php } // end if new encounter ?>
 //opener.window.top.left_nav.loadFrame('enc2', 'RTop', '<?php echo $nexturl; ?>');
 ...
 window.top.left_nav.setRadio(window.name, 'dem');
 window.top.left_nav.loadFrame2('dem1', window.name, 'patient_file/summary/demographics.php?frm_enc=true&visit=1');
