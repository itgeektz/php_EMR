...
function cancelClicked() {
 ...
 if (window.name == 'RBot') {
  ...
  //window.name = "RTop";
  //if((opener.window.location.href.indexOf('demographics.php') < 0)){
    opener.window.top.left_nav.setRadio(window.name, 'dem');
    opener.window.top.left_nav.loadFrame('dem1', window.name, 'patient_file/summary/demographics.php');
 //}
}
...
<div>
  ...
  <?php if ($GLOBALS['concurrent_layout']) {
    ...
    ?>
      <a href="<?php echo "$rootdir/patient_file/summary/demographics.php"; ?>"
        class="css_button link_submit" onClick="opener.window.top.restoreSession();window.close();"><span><?php echo xlt('Cancel'); ?></span></a>
  <?php } else { ?>
	  	 ...
	  	 <tr<?php if ($GLOBALS['athletic_team']) echo " style='visibility:hidden;'"; ?>>
     		...
     		<td class='text'>
		      <select name='pc_catid' id='pc_catid'>
				...
				<?php
 					$cres = sqlStatement("SELECT pc_catid, pc_catname  FROM openemr_postcalendar_categories where pc_catid in (12,13) ORDER BY pc_catid");
 					 while ($crow = sqlFetchArray($cres)) 
 					 {
  						$catid = $crow['pc_catid'];
  						//if ($catid) continue;
  						echo "<option value='" . attr($catid) . "'";
						if ($viewmode && $crow['pc_catid'] == $result['pc_catid']) echo " selected";echo ">" . text(xl_appt_category($crow['pc_catname'])) . "</option>\n";
 						}
				?>
      			</select>
     		</td>
...
<?php
if (!$viewmode) { ?>
 function duplicateVisit(enc, pid, datestr) {
    if (!confirm('<?php echo xl("A visit already exists for this patient today. Click Cancel to open it, or OK to proceed with creating a new one.") ?>')) {
            ...
            window.close();
            opener.window.top.restoreSession();
            window.name = 'RTop';
            opener.window.top.left_nav.setEncounter(datestr, enc, window.name);
          //  opener.window.top.left_nav.setRadio(window.name, 'enc');
          //  opener.window.top.left_nav.loadFrame('enc2', window.name, 'patient_file/encounter/encounter_top.php?set_encounter=' + enc);
            opener.window.top.left_nav.setRadio(window.name, 'dem');
            opener.window.top.left_nav.loadFrame2('dem1', window.name, 'patient_file/summary/demographics.php?opener_save=true&set_encounterid='+enc+'&set_pid='+pid);
            return;
        }
...
<?php
  ...
  if (!empty($erow['encounter'])) {
    ...
    echo "duplicateVisit('" . $erow['encounter'] . "','" . $pid . "', '" .
