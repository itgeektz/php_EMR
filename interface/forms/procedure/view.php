...
function get_service_name($id){
	$res = sqlQuery("SELECT service_name FROM rate_master WHERE id = '$id'");
	return $res['service_name'];
}

$obj = formFetch("form_procedure", $_GET["id"]);
...
<body class="body_top">
	...
	<form method="POST" action="<?php echo $rootdir;?>/forms/procedure/save.php?mode=update&id=<?php echo $_GET["id"];?>"   style="padding-top: 20px;" name="my_form" onsubmit="return top.restoreSession()" >
 		<div style="width: 70%;float:left;">
	        <table  width='100%' class='proctable' style="padding-left:20px; padding-bottom: 15px;">
				    <tr style="background: lightslategrey none repeat scroll 0 0; color:white;height:35px;" >
					...
					<?php
				   $service_name = json_decode($obj{"service_name"});
				   $service_code = json_decode($obj{"service_code"});
				   $comments =  json_decode($obj{"comments"});
				   $pat_cat = json_decode($obj{"pat_cat"});
				   $rate_cat = json_decode($obj{"rate_cat"});
				   $rate = json_decode($obj{"rate"});
  				...
  				<tr class="tr_clone">
    				<td valign='top' style="padding-top: 22px;width:400px;">
    					<input type='text' size='50' name='service_name[]' id='service_name_0' class='service_name' style='width:100%;' value='<?php echo $service_name[$i] ?>' />
    					<div id="suggesstion-box" class='suggesstion-box' style="position: absolute;"></div>
    				 </td>
    				<td style="width:400px;">
		     			<textarea  name='comments[]' style='width:100%;' id='comments_0' class='comments'><?php echo $comments[$i]; ?></textarea>
		      		</td>
				    ...
				    <input type="hidden" name="service_code[]" class="service_code" id="service_code" value="<?php echo $service_code[$i]; ?>">
    				<input type="hidden" name="pat_cat[]" class="pat_cat" id="pat_cat" value="<?php echo $pat_cat[$i]; ?>">
    				<input type="hidden" name="rate_cat[]" class="rate_cat" id="rate_cat" value="<?php echo $rate_cat[$i] ?>">
    				<input type="hidden" name="rate[]" class="rate" id="rate" value="<?php echo $rate[$i]; ?>">
				 </tr>
				 <?php } ?>
			</table>
		...
		<span style="float:left;"> 
  			<a href="<?php echo "$rootdir/patient_file/summary/demographics.php?opener_save=true";?>" class="link" onclick="top.restoreSession()"><input  style="width: 70px !important; height: 30px !important;" type="button" value="Cancel"></a>
		     	...
		     	<input type="hidden" name="update_id[]" class="update_id" id="update_id" value="<?php echo $obj['id']; ?>">
...
$(document).on('click','.del', function() {
 	...
 	var count = tbl_length; 
      ...
      {	
    	    ...
    	    $('.proctable .tr_clone:first').find('.service_name').val('');
    	    $('.proctable .tr_clone:first').find('.service_code').val('');
	    	$('.proctable .tr_clone:first').find('.pat_cat').val('');
	    	$('.proctable .tr_clone:first').find('.rate_cat').val('');
	    	$('.proctable .tr_clone:first').find('.rate').val('');
	    	$('.proctable .tr_clone:first').find('.comments').val('');
