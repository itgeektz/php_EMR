...
class C_FormVitals extends Controller {
    ...
    function __construct($template_mod = "general") {
    	...
    	$this->assign("DONT_SAVE_LINK",$GLOBALS['webroot'] . "/interface/patient_file/summary/demographics.php?opener_save=true");
      ...
      $sql = "SELECT fv.*, fe.date AS encdate " .
        ...
        $results[$i]['note'] = $result->fields['note'];
    		...
    		$results[$i]['head_circ'] = $result->fields['head_circ'];
				$results[$i]['oxygen_method'] = $result->fields['oxygen_method'];
				$results[$i]['oxygen_amt'] = $result->fields['oxygen_amt'];
				$results[$i]['abg'] = $result->fields['abg'];
				$results[$i]['fbg'] = $result->fields['fbg'];
