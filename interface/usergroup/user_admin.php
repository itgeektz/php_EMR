...
<br>
<FORM NAME="user_form" METHOD="POST" ACTION="usergroup_admin.php" target="_parent" onsubmit='return top.restoreSession()' enctype="multipart/form-data">
...
?>
  ...
  <tr id="qualification-tr" style="display:none">
    <td><span class="text"><?php  xl('Qualification','e'); ?>: </span></td>
    <td>
      <select name="qualification">
          <option value="0"><?php echo xl('Select Qualification','e') ?></option>
          <?php
        $fres = sqlStatement("select option_id,title from list_options where list_id='qualification_list' order by title");
        if ($fres) {
          for ($iters = 0;$frow = sqlFetchArray($fres);$iter++)
            $result[$iters] = $frow;
          foreach($result as $iters) {
			 $sel = ($iters["option_id"] == $iter["qualification"] ) ? 'selected="selected"' : '';	
              echo '<option value='.$iters["option_id"].'>'.$iters["title"].'</option>';
          }
        }
        ?>
      </select>
    </td>
    <td><span class="text"><?php  xl('Designation','e'); ?>: </span></td>
    <td><input type="entry" name="designation" style="width:120px;" value="<?php echo $iter["designation"];?>"></td>
  </tr>
  <tr id="UploadSignature-tr" style="display:none">
      <td><span class="text"><?php  xl('UploadSignature','e'); ?>: </span></td>
    <td colspan="2">
		<input type="file" name="signature"   value="Upload">
		<span><?php echo $iter["signature"];?></span>
		<input type="hidden" name="isdoctor" id="isdoctor" value="0">
	</td>
  </tr>
...
$(document).ready(function(){
    $("#cancel").click(function() {
	 ...
	 <?php if (($username_acl_groups) && in_array('Physicians',$username_acl_groups)) { ?>
	 	   $("#qualification-tr").css("display","");
            $("#UploadSignature-tr").css("display","");
            $("#isdoctor").val("1");
	 <?php } ?>
	 $("#access_group_id").change(function() {
        var isdoc=$(this).val();
        
       if (~isdoc.indexOf("Physicians"))
       {
            $("#qualification-tr").css("display","");
            $("#UploadSignature-tr").css("display","");
            $("#isdoctor").val("1");
       ...
       {
          $("#qualification-tr").css("display","none");
          $("#UploadSignature-tr").css("display","none");
          $("#isdoctor").val("0");
