...
if (isset($_POST["privatemode"]) && $_POST["privatemode"] =="user_admin") {
    if ($_POST["mode"] == "update") {
	  ...
	  if ($_POST["qualification"]) {
	  	 sqlStatement("update users set qualification='".$_POST["qualification"]."' where id= ? ", array($_POST["id"]));	
      }
      if ($_POST["designation"]) {
	  	 sqlStatement("update users set designation='".$_POST["designation"]."' where id= ? ", array($_POST["id"]));	
     }
     if (!empty($_FILES["signature"]["name"])) {
            $signature=time()."_".addslashes($_FILES["signature"]["name"]);
            //  $file_ext=strtolower(end(explode('.',$_FILES['image']['name'])));
            // $expensions= array("jpeg","jpg","png","pdf");
            $file_tmp=$_FILES["signature"]["tmp_name"];
           
             move_uploaded_file($file_tmp,$GLOBALS["srcdir"]."/doctort_signature/".$signature);
             sqlStatement("update users set signature='".$signature."' where id= ? ", array($_POST["id"]));  
     }
...
if (isset($_POST["mode"])) {
  if ($_POST["mode"] == "new_user") {
    ...
    if($success)
    {
      ...
      // start channge to me end
      if($_POST["isdoctor"])
      {
        if ($_POST["qualification"]) {
          
          $qry="update users set qualification = '".$_POST["qualification"]."' where username = '" . trim(formData('rumple')) . "'";
            sqlQuery($qry);
        }
         if ($_POST["designation"]) {
         
          $qry="update users set designation = '".$_POST["designation"]."' where username = '" . trim(formData('rumple')) . "'";
            sqlQuery($qry);
        }
        if (!empty($_FILES["signature"]["name"])) {
            $signature=time()."_".addslashes($_FILES["signature"]["name"]);
          ...
          $qry="update users set signature = '".$signature."' where username = '". trim(formData('rumple')) ."'";
            sqlQuery($qry);

        }
      }
    //start change to me end
...
<?php
 $sqlqry='SELECT ga.value, ag.name
       FROM gacl_groups_aro_map gam
       LEFT JOIN `gacl_aro` ga ON gam.aro_id = ga.id
       LEFT JOIN gacl_aro_groups ag ON ag.id = gam.group_id';
       $usergroupdetail = sqlStatement($sqlqry);
        $user_group_map=array();
        while($row=sqlFetchArray($usergroupdetail))
        {
            $key=$row['value'];
            $value=$row['name'];
          $user_group_map[$key]=$value;
        }
?>
    <select name="groupname" id="groupname" class="grou-nmae" onchange='submit()'>
      <option value=""><?php xl('All Group','e'); ?></option>
    <?php
      $sel_group=array_unique($user_group_map);
...
<table cellpadding="1" cellspacing="0" class="showborder">
    ...
    <th width="200px"><b><span class="bold"><?php xl('Group','e'); ?></span></b></th>
...
foreach ($result4 as $iter) {
  ...
  if(isset($_POST['groupname']))
  {
    ...
    if($_POST['groupname']==$user_group_map[$iter["username"] ])
    {
  ...
  print "<tr height=20  class='text' style='border-bottom: 1px dashed;'>
	...
	<td align='left'><span class='text'>" .$user_group_map[$iter{"username"} ] . "</span>&nbsp;</td>
