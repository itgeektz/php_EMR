...
class C_Document extends Controller {
	...
	function upload_action_process() 
	{
		...
		if ($_POST['process'] != "true")
			...
			$encounter_id = 0;
			if (isset($_GET['encounter_id']) && !$couchDB) 
			{
				$encounter_id = $_GET['encounter_id'];
			}
			else if (is_numeric($_POST['encounter_id'])) 
			{
			...
			$d = new Document();
			$login_name=sqlQuery("select username from users where id=".$_SESSION['authUserID']);
			...
			if( count($_FILES['file']['name']) > 0)
			{
				...
				{
					$fname = $encounter_id ."_uploaded_by_".$login_name['username']."_".$value;
					$err = "";
					...
					if ($_FILES['file']['error'][$key] > 0 || empty($fname) || $_FILES['file']['size'][$key] == 0)
					{
						...
						if ($_FILES['file']['size'][$key] == 0)
						{
						...
						}else
							{
								...
								{
									$fname =$encounter_id."_uploaded_by_".$login_name['username']."_".$_POST['destination'];
								}
