...
function InsertEvent($args,$from = 'general') {
  ...
  //echo '<pre>';print_r($args);die;	
    ...
    $pc_eid = sqlInsert("INSERT INTO openemr_postcalendar_events ( " .
			...
			",pc_pname,pc_pnumber,pc_page,pc_psex,pc_plocation,pc_pemail) VALUES (?,?,?,?,?,NOW(),?,?,?,?,?,?,?,?,?,?,?,?,?,1,1,?,?,?,?,?,?,?,?,?)",
			...
			$args['locationspec'],(int)$args['facility'],(int)$args['billing_facility'],$form_room,$args['pc_pname'],$args['pc_pnumber'],$args['pc_page'],$args['pc_psex'],$args['pc_plocation'],$args['pc_pemail'])
		);
