...
function ttMake() {
 ...
 $.get(ttUrl, function(data) {
  ...
  var x = ttGetX(ttElem);
  var dw = window.innerWidth ? window.innerWidth - 100 : document.body.clientWidth;
...
function ttMake_new() {
 ...
 top.restoreSession();
  ...
  if (!ttWantContent) return;
  ttobject = document.getElementById("tooltipdiv");
  var data = document.getElementById(vvElem).innerHTML;
  ttobject.innerHTML =data ;
  var x = ttGetX(ttElem);
...
// onmouseover handler
function ttMouseOverNew(elem, formname){

  
 ttClearTimer();
 ttElem = elem;
 vvElem=formname;
 ttTimerId = setTimeout("ttMake_new()", 250);
 return false;
}
