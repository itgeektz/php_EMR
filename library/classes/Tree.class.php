...
class Tree {
	...
	*/
	function add_node($parent_id,$name,$value="") {
	  ...
	  //insert new category in template categories table for custom templates
	  if($parent_id == 2){
	   $template_cate_sql="insert into template_categories set title ='".$name."', value='".strtolower($name)."', status=1";
	  
	   $this->_db->Execute($template_cate_sql) or die("Error:  $template_cate_sql :: " . $this->_db->ErrorMsg());
	  }
